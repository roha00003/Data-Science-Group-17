...
async def predict(data: PatientInput):
    ...
    for code, percentage, median in procedure_codes:
        ...
        result.append({
            ...
            "total_costs": round(float(pred[0][0]) - percentage * (float(median) - float(pred[0][0])), -2) if round(float(pred[0][0]) - percentage * (float(median) - float(pred[0][0])), -2) > 0 else median,  # round to nearest 100
